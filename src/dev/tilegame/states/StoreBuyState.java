...
public class StoreBuyState extends State {
    /**item is the Item a user can buy.*/
    private Item item;
     ...
     * @param inItem item to be bought.
     */
    public StoreBuyState(final Manager inManager, final Item inItem) {
        super(inManager);
        this.item = inItem;
        this.player = inManager.getPlayer();
    }
        ...
        if (manager.getMouseManager().getLeftPress()) {
            ...
            if ((x > 188 && x < 344) && (y > 583 && y < 622)) {
                if ((player.getMoney() >= item.getCost())
                        && (player.getInventory().getSize() < 12)) {
    ...
    public void render(final Graphics g) {
        ...
        if ((player.getMoney() < item.getCost())
                || (player.getInventory().getSize() > 11)) {
        ...
        g.drawString(item.getName(),
                (328 - fm.stringWidth(item.getName())) / 2, 458);
        ...
        for (String line : item.getDescription().split("\n")) {
            g.drawString(line, ((270 - fm.stringWidth(line)) / 2), y
